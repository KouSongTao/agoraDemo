...
class AgoraUploader {
    ...
    static func startBroadcast(to channel: String) {
        ...
        let channelId = UserDefaults.init(suiteName: "group.com.zgtfintech")?.string(forKey: "channelId")
        let token = UserDefaults.init(suiteName: "group.com.zgtfintech")?.string(forKey: "token")
        let uid = UserDefaults.init(suiteName: "group.com.zgtfintech")?.object(forKey: "uid")

        sharedAgoraEngine.joinChannel(byToken: token, channelId: channelId!, info: nil, uid: uid as! UInt, joinSuccess: nil)

    }
    
    static func finishedBroadcast() {
